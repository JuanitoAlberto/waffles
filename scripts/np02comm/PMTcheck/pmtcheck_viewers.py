"""Visores interactivos (ipywidgets) para inspeccionar waveforms.

Antes eran celdas comentadas del notebook; ahora son funciones que se
llaman bajo demanda desde cualquier notebook.
"""
...
def browse_anomalous(
    ...
    """Visor de las waveforms mas anomalas de un canal, ordenadas por anomaly score."""
    ...
    anomaly_scores = np.sqrt(
        robust_z([m["pre_rms"] for m in metrics]) ** 2
        + robust_z([m["peak_tick"] for m in metrics]) ** 2
        + robust_z([m["peak_amp"] for m in metrics]) ** 2
        + robust_z([m["integral"] for m in metrics]) ** 2
    )

    for m, score in zip(metrics, anomaly_scores):
        m["anomaly_score"] = score

    n_to_show = min(n_keep, len(selected_waveforms))
    anomaly_order = np.argsort(anomaly_scores)[::-1][:n_to_show]

    print(f"Waveforms analizadas: {len(selected_waveforms)}")
    print(f"Waveforms mostrables en el slider: {n_to_show}")
    print(f"Preparacion terminada en {time.time() - t0:.2f} s")

    def plot_anomalous_waveform(anomaly_rank=0):
        actual_idx = anomaly_order[anomaly_rank]
        wf = selected_waveforms[actual_idx]
        ...
        info_text = (
            f"anomaly rank = {anomaly_rank} / {n_to_show - 1}\n"
        ...
        plt.title(f"All waveforms | Endpoint {endpoint}, channel {channel}")
        plt.legend(fontsize=8, loc="upper right")
        plt.grid(alpha=0.3)
        plt.show()

    return interact(
        plot_anomalous_waveform,
        anomaly_rank=IntSlider(
            min=0, max=n_to_show - 1, step=1, value=0,
...
def browse_channel_waveforms(wfset, endpoint=110, analysis_label=ANALYSIS_LABEL):
    """Visor interactivo de waveforms por canal (canal con dropdown, indice con slider)."""
    ...
    def browse_channel(channel):
        ...
        def show_waveform(idx):
            ...
            plt.title(f"Endpoint {endpoint} | Channel {channel} | WF {idx}")
            plt.grid(True)
            plt.show()


def browse_cluster_events(clusters):
    """Visor de waveforms individuales de cada cluster Qfast/Qslow.
